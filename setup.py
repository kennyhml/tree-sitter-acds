...
class Build(build):
    def run(self):
        if path.isdir("queries"):
            dest = path.join(self.build_lib, "tree_sitter_acds", "queries")
...
setup(
    ...
    package_data={
        "tree_sitter_acds": ["*.pyi", "py.typed"],
        "tree_sitter_acds.queries": ["*.scm"],
    },
    ext_package="tree_sitter_acds",
    ext_modules=[
        Extension(
            name="_binding",
            sources=[
                "bindings/python/tree_sitter_acds/binding.c",
                "src/parser.c",
            ],
            define_macros=[
